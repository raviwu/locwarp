...
- An id collision is resolved **per merge unit**, not per record (change G) — see below
...
### Three rules that stop an edit being reverted (E1 + F, 2026-08-26; G, 2026-08-27)

The first two shrink what reaches the merge; the third changed the merge itself. All three are the kind of invariant a later refactor would "simplify" away:

- **The catalog force-sync resolves per field, not per record.** `import_catalog` compares three sides per field — `base` (the catalog values this machine last applied, `~/.locwarp/catalog_baseline.json`), `ours`, `theirs` — via the pure `domain/catalog_merge.resolve_record`, and only takes the catalog's value for a field the user never edited. The baseline is **local to one machine and never in `sync_folder`** (each Mac bootstraps its own; a synced baseline would itself need merge semantics). With no baseline file, `base := theirs` per field, so every already-diverged value is preserved. The live record is re-stamped **only** when a field came from theirs, `enrich_bookmark` reported a change, or the id is in the tombstone set (in-memory **union** on-disk — an unstamped record would otherwise be killed by an unreconciled peer tombstone inside the same `_save()`). An unconditional re-stamp would make Refresh itself a revert trigger for the other Mac. `country_code` / `timezone` / `city` / `region` are **not** merged and the catalog never writes them onto an existing record — `enrich_bookmark` is their sole author. Since G that re-stamp is **per unit**: `Resolution.taken` names the fields that came from theirs, so correcting a `name` leaves `address` reading as of the user's last edit, and an enrich-only pass bumps the record (needed to out-vote a tombstone) while stamping no unit at all. `import_json` and `force_seed` keep the blind-overwrite branch (`_upsert_items(resolver=None)`), which also **clears `field_updated_at`** — every unit is as of now there by definition.
- **The merge resolves per unit, not per record (change G).** Records carry an additive `field_updated_at: dict[str, str]`, and `merge_records` picks each *merge unit* independently: newer unit stamp wins; tie → newer record `updated_at`; still tied → a symmetric content sort (never argument order). The unit tables live in `domain/store_merge.py` — `BOOKMARK_MERGE_UNITS` (`name` / `coords` / `address` / `category_id` / `last_used_at`), `CATEGORY_MERGE_UNITS`, `ROUTE_MERGE_UNITS`, `ROUTE_CATEGORY_MERGE_UNITS`. **Units, not raw fields:** `lat`+`lng` move together, the geo four (`country_code` / `timezone` / `city` / `region`) ride with `coords` because `enrich_bookmark` derives them from it, and a route's distance cache rides with `geometry`. Writers call `stamp_units(record, changed, now, units)`, which stamps the changed units at `now` **and backfills every untouched unit with the record's PREVIOUS `updated_at`** — without that backfill an unstamped unit falls back to the fresh record stamp and goes on clobbering, i.e. G would buy nothing. Enrichment stamps no unit. **No migration:** an unstamped record reads as `updated_at` for every unit, so a pre-G store and an old build (which strips the unknown key via pydantic `extra='ignore'`) both degrade to exactly the old whole-record LWW, never worse. Because the merge is now commutative all the way down, a caller whose contract is positional must say so with `prefer_left_on_exact_ties` — `services/sync_merge.py` ("local wins" on the one-shot migration) and `merge_backup.py` ("the live store wins; a backup only fills gaps") are the two.
- **`PUT` is a partial update, and a no-op `PUT` writes nothing.** `PUT /api/bookmarks/{id}` takes `BookmarkUpdate` and `PUT /api/bookmarks/categories/{id}` takes `BookmarkCategoryUpdate` (all-Optional, forwarded as `model_dump(exclude_unset=True)`) — an omitted key means "leave unchanged", `""` still clears. `update_bookmark` **and `update_category`** both diff before mutating and, when nothing differs, return without re-stamping `updated_at` and **without calling `_save()`**; a Save that changed nothing must never reach the merge carrying a fresh timestamp. The category half matters because the category dialog submits an empty patch when the user opened it and changed nothing — pinned by `test_put_category_with_an_empty_body_writes_nothing` and its two siblings. `_validate_date_range` treats `None` (omitted) exactly like `""`, so an omitted date is a 200, not a 500.
