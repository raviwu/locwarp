...
**Three rules that stop an edit being reverted (E1 + F, 2026-08-26; G, 2026-08-27).** The first two shrink what reaches the merge; the third changed the merge itself. All three are the kind of invariant a later refactor would "simplify" away:

- **The catalog force-sync resolves per field.** `import_catalog` compares `base` (the catalog values this machine last applied, `~/.locwarp/catalog_baseline.json` — local-only, never in `sync_folder`), `ours` and `theirs` via the pure `domain/catalog_merge.resolve_record`, and takes the catalog's value only for a field the user never edited. No baseline file ⇒ `base := theirs` per field, so every already-diverged value is preserved. The live record is re-stamped **only** when a field came from theirs, `enrich_bookmark` reported a change, or the id is in the tombstone set (in-memory **union** on-disk — otherwise an unstamped record dies to an unreconciled peer tombstone inside the same `_save()`). An unconditional re-stamp would make Refresh itself a revert trigger for the other Mac. `country_code` / `timezone` / `city` / `region` are not merged and the catalog never writes them onto an existing record — `enrich_bookmark` is their sole author. Since G that re-stamp is per unit (`Resolution.taken` names the fields taken from theirs), so correcting a `name` leaves `address` reading as of the user's last edit and an enrich-only pass stamps no unit. `import_json` / `force_seed` keep the blind-overwrite branch (`_upsert_items(resolver=None)`), which also clears `field_updated_at`.
- **The merge resolves per unit, not per record (change G).** Records carry an additive `field_updated_at: dict[str, str]`; `merge_records` picks each *merge unit* independently (newer unit stamp → newer record `updated_at` → symmetric content sort, never argument order). Unit tables in `domain/store_merge.py`: `BOOKMARK_MERGE_UNITS` (`name` / `coords` / `address` / `category_id` / `last_used_at`), `CATEGORY_MERGE_UNITS`, `ROUTE_MERGE_UNITS`, `ROUTE_CATEGORY_MERGE_UNITS`. Units, not raw fields: `lat`+`lng` move together, the geo four ride with `coords`, a route's distance cache rides with `geometry`. Writers call `stamp_units(record, changed, now, units)`, which stamps changed units at `now` **and backfills untouched units with the record's PREVIOUS `updated_at`** — without the backfill an unstamped unit falls back to the fresh record stamp and G buys nothing. No migration: an unstamped record reads as `updated_at` for every unit, so a pre-G store and an old build (pydantic `extra='ignore'` strips the key) both degrade to the old whole-record LWW, never worse. The merge is now commutative all the way down, so a positional contract must say so via `prefer_left_on_exact_ties` (`services/sync_merge.py`, `merge_backup.py`).
- **`PUT` is a partial update, and a no-op `PUT` writes nothing.** `PUT /api/bookmarks/{id}` and `PUT /api/bookmarks/categories/{id}` take all-Optional request models forwarded as `model_dump(exclude_unset=True)`: an omitted key means "leave unchanged", `""` still clears, and `_validate_date_range` treats `None` like `""` (so an omitted date is a 200, not a 500). `update_bookmark` and `update_category` both diff before mutating and, when nothing differs, return without re-stamping `updated_at` and without calling `_save()` — a Save that changed nothing must never reach the merge carrying a fresh timestamp.
