...
**Status (2026-06-22):** Phase 0 + Phase 1 + Phase 2 (C / spec-literal) merged (2026-06-20); **Phase 3 (movement-math carve-out) IMPLEMENTED on branch `chore/clean-arch-p3` (2026-06-22), pending hardware smoke + merge**. Inward-only rings enforced; geocode `GeocodeError`s mapped at the boundary; last `infra→api` edge killed; all `from main import app_state` retired from non-test code; api→api broadcasts via injected `EventPublisher`; **six import-linter contracts ENFORCED (`6 kept, 0 broken`)** — Phase 3 added `no-domain-imports-outer` guarding the new pure `domain/movement.py` (EtaTracker + `build_resume_snapshot` + RouteInterpolator; the last was relocated from `services/`, killing the final `core→services` interpolator edge). Phase 3 kept `resume_from_snapshot` warn-and-return (behavior-freeze) and deferred the `RouteService` `core→services` edge. Phases 4–5 deferred. **Do not start Phases 4–5 without explicit approval from Ravi.**
