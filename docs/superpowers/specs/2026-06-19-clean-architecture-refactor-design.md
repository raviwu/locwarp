...
### Phase 4 — Repository around the CRDT store + frontend god-component decomposition — **SPLIT: P4a (backend) DONE 2026-06-22; P4b (frontend) deferred** — ~10 commits

> **SPLIT + P4a DONE note (2026-06-22):** This phase covered two independent subsystems, so it was split. **P4a (backend repository) IMPLEMENTED on `chore/clean-arch-p4a`** (8 commits): inverted the bookmark/route **file-I/O** behind thin `BookmarkRepository`/`RouteRepository` Protocols (`domain/ports/`) implemented by a generic `JsonStore` (`infra/persistence/json_store.py`), built ONLY at the composition root (`bootstrap/factories.py` + the 3 manager-construction sites) so services never import infra; moved the pure `merge_stores` CRDT rule to `domain/store_merge.py` (re-export shim in services); added a shared `force_seed_items` primitive encoding the empty-`updated_at` tombstone pitfall (`import_catalog` uses it). The watcher state machine + `_store_lock` + mtime STAY on the managers (each keeps its own divergent `_watcher_tick`). **Deviation from §4.4-literal:** `merge_stores` went to `domain/`, not `infra/persistence` (it's a pure rule, consistent with P3's `domain/movement.py`) — owner-approved. **The RouteManager lock the original §4.4 implied was NOT added** — adversarial review empirically proved no race (route `_watcher_tick` never writes disk). No external behavior change (pure structural inversion); backend 871→876 (+5 tests); **7 import-linter contracts kept/0 broken** (added `no-infra-imports-fastapi`). **P4b (frontend god-component decomposition) is a separate later brainstorm+design+plan cycle** (needs the cross-slice shared-state architecture decision + a Leaflet test-double strategy; MapView/App.tsx/BookmarkList are ~0% unit-covered).
