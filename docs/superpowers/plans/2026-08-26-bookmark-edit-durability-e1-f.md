...
- **Not extended to `update_category`.** Same hazard in principle, but no measured symptom and the category path is already fenced as separable (Task 10). Recorded in §11 rather than fixed.
...
4. **`update_category` keeps the unconditional re-stamp.** §6.6's no-op guard is applied to `update_bookmark` only. `update_category` has the same shape and no measured symptom; if Task 10 ships, a no-op category Save still bumps the stamp and can replace the other Mac's copy. Deliberately left, to keep the guard's blast radius to the one method with a reported bug.
